...
void Draw(HWND hWnd)
{
    ...
    if (globalButtonDown) {
        ...
        const int nDeltaX = currCursorPos.x - prevCursorPos.x;
		const int nDeltaY = currCursorPos.y - prevCursorPos.y;
        if (nDeltaX < -1 || nDeltaX > 1 || nDeltaY < -1 || nDeltaY > 1) {
            curWindowRect.left += nDeltaX;
			curWindowRect.top += nDeltaY;
...
int main()
{
	...
	if (!InitInstance (hInst, SW_SHOW, &hWnd))
	{
		return FALSE;
	}
